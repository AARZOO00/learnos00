  ...
  return (
    <main className="min-h-screen px-6 py-10 md:px-16" style={{ background: 'var(--bg-primary)' }}>
      ...
      <motion.section
        ...
        className="mx-auto max-w-5xl rounded-3xl border border-cyan-400/15 bg-slate-950/70 p-10"
      >
        <FileText className="mb-6 text-cyan-300" size={42} />
        ...
        {['Use License', 'Disclaimer', 'User Responsibilities'].map((title) => (
          <div key={title} className="mt-8 rounded-2xl border border-cyan-400/10 bg-slate-900/40 p-6">
            <h2 className="text-xl font-bold text-slate-100">{title}</h2>
            <p className="mt-3 leading-7 text-slate-400">
              By using LearnOS, users agree to use the platform responsibly for educational and personal learning purposes.
            </p>
          </div>
        ))}
      </motion.section>
